  ...
  return (
    <>
      {myChats?.map((user, index) => (
        <>
          <div key={index} onClick={() => getUserDetails(user.name, user.status)} style={{ display: "flex", flexDirection: "row", alignItems: "center", margin: 10, padding: 8, borderRadius: 8, cursor: "pointer", backgroundColor: "white", boxShadow: " 0 .2rem 0.5rem rgba(0,0,0,.15)" }}>
