...
const CurrentVisitor = () => {
  ...
  const [currentVisitorList, setCurrentVisitorList] = useState([]);
  const [itemCount, setItemCount] = useState(10);

  const getCurrentVisitors = () => {
    const { uid } = auth.currentUser
    RealTimeDb.ref(`users/${subdomain}/`).limitToFirst(itemCount).orderByChild('exhibition_id').equalTo(exhibition_id).on("value", (snapshot) => {
      let users = [];
      snapshot.forEach((snap) => {
        if (snap.val().exhibitor != true && snap.val().uid != uid) {
          users.push({
    ...
    <>
      <div className="attendeeDiv" onScroll={(e) => loadMore(e)}>
        <div style={{ padding: 10, backgroundColor: "white", boxShadow: ".2rem rgba(0,0,0,.15)" }}>
          <span style={{ color: "#5B5B5B", fontSize: 18, fontWeight: 500 }}>Current Visitors</span>
        </div>
        {currentVisitorList?.map((user, index) => (
          <>
